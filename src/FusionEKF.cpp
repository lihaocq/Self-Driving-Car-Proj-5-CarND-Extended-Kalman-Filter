...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  if (!is_initialized_) {
    ...
    // initialize the state variable with first measurement
    ekf_.x_ << px, py, range_rate*cos(bearing), range_rate*sin(bearing);

    }
    else if (measurement_pack.sensor_type_ == MeasurementPackage::LASER) {
      // TODO: Initialize state.     ** that is initialize x.
  ...
  //* do: update state transition matrix F
  ekf_.F_ << 1, 0, dt, 0,
  			 0, 1, 0, dt,
             0, 0, 1, 0,
             0, 0, 0, 1;

  //* do: Update the process noise covariance matrix.
  double noise_ax = 9.0;
  double noise_ay = 9.0;
  ...
  if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR && radar_on) {
    // TODO: Radar updates
   VectorXd z;
...
}else if(measurement_pack.sensor_type_ == MeasurementPackage::LASER && laser_on){
  // TODO: Laser updates
  VectorXd z;
   z = measurement_pack.raw_measurements_;
   ekf_.H_ = H_laser_;
